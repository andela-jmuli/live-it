...
class RegisterUser(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            response = jsonify({'message': 'the username is already registered'})
            ...
            try:
                db.session.add(user)
                db.session.commit()
                response = jsonify({'message': 'new user successfully registered!'})
                response.status_code = 201
                return response
            except Exception :
                response = jsonify({'message': 'there was a problem while saving the data'})
